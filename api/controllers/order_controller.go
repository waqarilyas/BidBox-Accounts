...
func (server *Server) DeleteOrder(w http.ResponseWriter, r *http.Request, email string) {
	var order models.CancelOrderRequest
	...
	str, err := CancelOrder(api_key, secret_key, passphrase, &order)
	if err != nil {
		response.ERROR(w, http.StatusInternalServerError, err)
		return
	}
...
func CancelOrder(api_key string, secret_key string, passphrase string, order *models.CancelOrderRequest) (string, error) {
